...
linear_spike_in='/home/jinxin/projects/ecDNA_Project/ecDNA_scripts/src/spike_in/linear_spike_in/lamda_dna.fa'
circular_spike_in='/home/jinxin/projects/ecDNA_Project/ecDNA_scripts/src/spike_in/circular_spike_in/RBD_plasmid.fa'
myc_ecDNA='/home/jinxin/projects/ecDNA_Project/ecDNA_scripts/src/colo320dm/COLO320DM_Version2_ecDNA1_cycles_cycle1.bed'
Circlebed2AAbed='/home/jinxin/projects/ecDNA_Project/ecDNA_scripts/src/Circlebed2AAbed.R'
circleEnrichFilter='/home/jinxin/projects/ecDNA_Project/ecDNA_scripts/src/call_circle_region.sh'
...
while getopts "f:r:o:t:p:e:A:g:R:" options; do
    case ${options} in 
        f) fastq1=${OPTARG};;
        r) fastq2=${OPTARG};;
        o) OUTPUT=${OPTARG};;
...
java -jar ${trimmomatic} PE -threads ${nThreads} -phred33 ${fastq1} ${fastq2} \
...
cd ${PWD}/${OUTPUT}/
/gshare/xielab/chenjx/ecDNA_Project/scripts/src/call_circle_region.sh ${PWD}/${PREFIX}.dedup.bam ${PWD}/${PREFIX}_circle_filter_threshold_${threshold} ${threshold}
cd ..
...
if [${Reconstruction} == 'TRUE' ];then
# Circle Reconstruction(only on chr8 which contains MYC)
if [ ${AAcell} == 'pc3' ];then
    AA_path='/share/home/chenjx/data/ecDNA/PC3/PC3_dedup.bam'
elif [ ${AAcell} == 'colo' ];then
    AA_path='/gshare/xielab/chenjx/ecDNA_cell_line_reference/colo320dm/colo320dm/colo320dm.dedup.bam'
elif [ ${AAcell} == 'hela' ];then
    AA_path='/share/home/chenjx/data/ecDNA/HeLa/HeLa_dedup.bam'
fi
mkdir ${OUTPUT}/${PREFIX}_AA
mkdir ${OUTPUT}/${PREFIX}_AR
   # Circle Reconstruction with AmpliconArchitect
cp ${OUTPUT}/${PREFIX}.dedup.bam ${OUTPUT}/${PREFIX}_AA/${PREFIX}.dedup.bam
cp ${OUTPUT}/${PREFIX}.dedup.bam.bai ${OUTPUT}/${PREFIX}_AA/${PREFIX}.dedup.bam.bai
bedtools intersect -wa -a ${OUTPUT}/${PREFIX}_circle_filter_threshold_${threshold}/Regions_${PREFIX}.dedup.enriched.merged.b2bRefined.Counts.ThreshFinal.bed -b ${myc_ecDNA} > ${OUTPUT}/${PREFIX}_AA/Regions_${PREFIX}.ThreshFinal.bed
Rscript ${Circlebed2AAbed} ${OUTPUT}/${PREFIX}_AA/Regions_${PREFIX}.ThreshFinal.bed ${OUTPUT}/${PREFIX}_AA/${PREFIX}_circle_AA.bed    
cd ${OUTPUT}/${PREFIX}_AA
# /share/home/chenjx/tools/AmpliconArchitect/src/AmpliconArchitect.py --bam ${PREFIX}.dedup.bam --bed ${PREFIX}_circle_AA.bed --out ${PREFIX}_AA --ref ${ref} --cbam ${AA_path} --extendmode EXPLORE
/share/home/chenjx/tools/AmpliconArchitect/src/AmpliconArchitect.py --bam ${PREFIX}.dedup.bam --bed ${PREFIX}_circle_AA.bed --out ${PREFIX}_AA --ref ${ref} --extendmode EXPLORE --no_cstats --insert_sdevs 8
cd ..
   # Circle Reconstruction with AmpliconReconstruct
for file in $(find ${PREFIX}_AA/ -name "*_graph.txt");do
   graphe_file=${file##*/}
   amplicon_name=${graphe_file/%_graph.txt/}
   #generate cmap file for each amplicon 
   /share/home/chenjx/tools/AmpliconReconstructor/generate_cmap.py -r ${GENOME} -e DLE1 -g ${PREFIX}_AA/${graphe_file} -o ${PREFIX}_AA/${amplicon_name}
   #generate yaml file for each amplicon
   cat << EOF > ${PREFIX}_AA/${amplicon_name}.yaml
   ${amplicon_name}:
    path: ${PWD}

    graph: ${PREFIX}_AA/${graphe_file}

    contigs: /gshare/xielab/chenjx/ecDNA_Project/scripts/src/colo320dm/COLO320_DM.cmap

    cmap: ${PREFIX}_AA/${amplicon_name}.cmap
    ...
    xmaps: ~     
EOF
   #run AmpliconReconstructor(COLO320DM)
   /share/home/chenjx/tools/AmpliconReconstructor/AmpliconReconstructorOM.py -i ${PREFIX}_AA/${amplicon_name}.yaml --outdir ${PREFIX}_AR/ --nthreads ${nThreads} --plot_scores --visualize_scaffolds --CV_path /share/home/chenjx/tools/CycleViz
done 
cd ..
cd ..
fi